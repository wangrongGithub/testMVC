...
import cn.wr.SpringBoot.TestJPA.Entity.Department;
import cn.wr.SpringBoot.TestJPA.Entity.Role;
import cn.wr.SpringBoot.TestJPA.Entity.User;
import cn.wr.SpringBoot.TestJPA.MyJpaRepository.DepartmentRepository;
import cn.wr.SpringBoot.TestJPA.MyJpaRepository.RoleRepository;
import cn.wr.SpringBoot.TestJPA.MyJpaRepository.UserRepository;
import cn.wr.SpringBoot.TestJPA.model.UserQo;
...
public class UserController {
    ...
    @Autowired
    private final ThreadLocal<UserRepository> userRepository = new ThreadLocal<UserRepository>();
    @Autowired
    ...
    public String show(ModelMap model,@PathVariable Integer id) {
        User user = userRepository.get().findOne(id);
        model.addAttribute("user",user);
        return "user/show";
    }
    ...
    public Page<User> getList(UserQo userQo) {
        try {
            ...
            return userRepository.get().findByName(userQo.getName()==null?"%":"%"+userQo.getName()+"%", pageable);
        }catch (Exception e){
            e.printStackTrace();
        }
        return null;
